...
let _client: RadarrClient | null = null

export function initRadarrClient(): RadarrClient | null {
  const host = process.env.RADARR_HOST
  const apiKey = process.env.RADARR_API_KEY

  if (!host || !apiKey) return null

  _client = createRadarrClient({
    host,
...
export function createRadarrClientFromEnv(): RadarrClient | null {
  return initRadarrClient()
}
