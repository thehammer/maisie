...
export function createPlexClient(config: PlexConfig) {
  ...
  return {
    ...
    async getLibraries(): Promise<{ key: string; title: string; type: string; count: number }[]> {
    ...
    async getLibraryCount(key: string): Promise<number> {
      const data = await request(
        `/library/sections/${key}/all?X-Plex-Container-Start=0&X-Plex-Container-Size=0`,
      )
...
let _client: PlexClient | null = null

export function initPlexClient(): PlexClient | null {
  const host = process.env.PLEX_HOST
  const token = process.env.PLEX_TOKEN

  if (!host || !token) return null

  _client = createPlexClient({
    host,
...
export function createPlexClientFromEnv(): PlexClient | null {
  return initPlexClient()
}
